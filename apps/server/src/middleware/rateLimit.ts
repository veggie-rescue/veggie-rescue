  ...
  if (ipStore.has(ip)) {
    ...
    if (ipData.requestCount >= getRateLimit()) {
      ...
      res.setHeader(
        ...
        getRateLimit() - ipData.requestCount,
      );
      res.setHeader(
        'X-RateLimit-Reset',
        new Date(ipData.retryAfter).toString(),
      );
...
function attemptCleanup(): void {
  ...
  // Measured in ms
  if (
    Date.now() - lastIpStoreCheck <
    NUM_MINUTES_BETWEEN_CLEANUPS * 60 * 1000
  ) {
    return;
  }
